...
"""
print('--------------------------')
print('theta before foot contact: ',theta[i_begin-1])
print('theta foot contact: ',theta[i_begin])
print('theta after foot contact: ',theta[i_begin+1])
print('--------------------------')
print('y before foot contact: ',y[i_begin-1])
print('y foot contact: ',y[i_begin])
print('y after foot contact: ',y[i_begin+1])
print('--------------------------')
print('foot contact index: ',i_begin)
print('')
print('--------------------------')
print('theta before foot liftoff: ',theta[i_end-1])
print('theta foot liftoff: ',theta[i_end])
print('theta after foot liftoff: ',theta[i_end+1])
print('--------------------------')
print('y before foot liftoff: ',y[i_end-1])
print('y foot liftoff: ',y[i_end])
print('y after foot liftoff: ',y[i_end+1])
print('--------------------------')
print('foot liftoff index: ',i_end)
print('')
print('--------------------------')
print('stride length in x: ',stride_length)
"""
...
contact_velocity    = abs(theta[i_begin] - theta[i_end]) / time_ground
flight_velocity     = (2*np.pi - abs(theta[i_begin] - theta[i_end])) / time_air

"""
print('')
print('time spend in contact with ground (s): ',time_ground)
print('time spent in air (s): ',time_air)
print('velocity while in contact with ground (rad/s): ',contact_velocity)
print('velocity while in in air (rad/s): ',flight_velocity)
"""
...
for i in range(1,steps,1):
    # set the velocity to match whether we're on the ground or in the air
    if (y_aligned[i] <= -0.35+margin):
        
    else:
        
    
    if (abs(theta[i] - theta[i-1]) > 1):
        # transitioning from close to 0 radians to 2pi radians (hitting 0), gotta treat this one slightly differently
        theta_d[i] = ((theta[i] - 2*np.pi) - theta[i-1]) / time_step
    else:
        # any other angle is fine
        theta_d[i] = (theta[i] - theta[i-1]) / time_step
    theta_dd[i] = (theta_d[i] - theta_d[i-1]) / time_step
    ...
    if (y_aligned[i] > -0.35+margin and y_aligned[i-1] <= -0.35+margin):
        print('index of foot coming off ground: ',i)

# since the initial speed is zero, massive initial acceleration, so neglect (likely a result of the simulation method used)
torque[1] = 0
...
np.savetxt('jansen_simulation.csv',np.transpose(np.array([time,theta,theta_d,theta_dd,torque,Ms,Cs,Gs,Fs])))
